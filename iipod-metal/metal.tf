...
resource "equinix_metal_device" "machine" {
  ...
  user_data = templatefile("cloud-config.yaml", {
    ...
    values_cilium = base64encode(
      templatefile("./etc/cloud/values-cilium.yaml", {
        k8s_service_host = local.dns_zone
    }))
    deploy_k8s = base64encode(
      templatefile("./etc/cloud/deploy-k8s.tftpl", {
        username = "ii"
    }))
    elastic_network_config = base64encode(
      templatefile("./etc/network/interfaces.d/elastic.tftpl", {
        ip = local.elastic_ip
    }))
    # cilium_manifest      = base64encode(file("./etc/kubernetes/manifests/cilium.yaml"))
    # snoopdb_manifest     = base64encode(file("./etc/kubernetes/manifests/snoopdb.yaml"))
